...
contract KaiSignFast is ReentrancyGuard, AccessControl, Pausable {
    ...
    uint32 public constant DEFAULT_TIMEOUT = 15 minutes; // 15 minutes for Reality.eth questions (CHANGED FROM 48 hours)
    ...
    function _proposeSpec(bytes32 specID) internal {
        ...
        // Create Reality.eth question with 15 minute timeout
        ...
        spec.questionId = RealityETH_v3_0(realityETH).askQuestionWithMinBond{value: totalBond}(
            ...
            questionText,
            arbitrator,
            DEFAULT_TIMEOUT, // 15 minutes - will extend by 24h on each challenge
            0,
            0,
            minBond
        );
