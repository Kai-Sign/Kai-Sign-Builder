...
contract KaiSignFast is ReentrancyGuard, AccessControl, Pausable {
    ...
    uint32 public constant DEFAULT_TIMEOUT = 15 minutes; // 15 minutes for Reality.eth questions
    ...
    function _proposeSpec(bytes32 specID) internal {
        ...
        spec.questionId = RealityETH_v3_0(realityETH).askQuestionWithMinBond{value: totalBond}(
            ...
            questionParams,
            arbitrator,
            DEFAULT_TIMEOUT, // 15 minutes - will extend by minimum on each challenge
            0,
            0,
            minBond
        );
