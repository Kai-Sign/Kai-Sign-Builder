...
contract KaiSign is ReentrancyGuard, AccessControl, Pausable {
    ...
    function revealSpec(
        ...
        return _revealSpecInternal(commitmentId, ipfs, nonce, bytes32(0));
    }
    
    /**
     * @notice Reveal a spec with metadata content hash for proper validation
     * @param commitmentId The commitment ID from the commit phase
     * @param ipfs The IPFS CID or URI for the metadata
     * @param nonce The nonce used in commitment
     * @param contentHash The hash of the actual metadata content (not the URI)
     * @return specID The created spec ID
     */
    function revealSpecWithContentHash(
        bytes32 commitmentId,
        string calldata ipfs,
        uint256 nonce,
        bytes32 contentHash
    ...
    function handleResult(bytes32 specID) external nonReentrant whenNotPaused {
        ...
        if (specAccepted) {
            // Attest the metadata to the registry
            // Use the actual content hash if available, otherwise hash the URI
            bytes32 metadataHash;
            if (spec.metadataContentHash != bytes32(0)) {
                // Use the actual content hash for meaningful validation
                metadataHash = spec.metadataContentHash;
            } else {
                // Fallback to URI hash for backward compatibility
                metadataHash = keccak256(abi.encodePacked(spec.ipfs));
            }
