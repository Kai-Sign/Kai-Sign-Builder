...
export class BlobValidationService {
	...
	constructor(rpcUrl?: string) {
		...
		} else if (typeof window !== 'undefined' && (window as unknown as { ethereum?: ethers.Eip1193Provider }).ethereum) {
			this.provider = new ethers.BrowserProvider((window as unknown as { ethereum: ethers.Eip1193Provider }).ethereum);
		}
	}
...
4. You're checking on the wrong network (should be Sepolia)`
	...
	private validateBlobHashFormat(blobHash: string): BlobValidationResult {
		...
		if (!hexRegex.test(blobHash)) {
			...
			const errorMessage: string = (invalidChar !== null && invalidPos !== -1)
	 ...
	 */
	private async checkBlobExists(blobHash: string): Promise<{ exists: boolean; blobData?: { txHash: string; blockNumber: number; blobIndex: number; etherscanUrl: string } }> {
		if (!this.provider) {
		...
		try {
			...
			try {
				const raw: unknown = await (this.provider as ethers.JsonRpcProvider).send('eth_getBlobByHash', [blobHash]);
			...
			for (let i = 0; i < searchBlocks; i += 10) { // Check every 10th block for efficiency
				...
				try {
					...
					for (const tx of txs) {
						const hasBlobHashes = (tx as unknown as { type?: number; blobVersionedHashes?: readonly string[] | string[] | null | undefined });
						if (hasBlobHashes?.type === 3 && hasBlobHashes.blobVersionedHashes) {
							const blobHashes = hasBlobHashes.blobVersionedHashes as readonly string[];
			...
			try {
				const filter = {
					...
					topics: [
						'0x' + ethers.keccak256(ethers.toUtf8Bytes('LogRevealSpec(address,bytes32,bytes32,bytes32,address,uint256)')).slice(2)
					]
				};
				...
				for (const log of logs) {
					...
					const logBlobHash = topics && topics.length >= 3 ? topics[2] : undefined;
				...
				console.log('Log search failed:', logError instanceof Error ? logError.message : String(logError));
			}
	 ...
	 */
	async getBlobDataFromEtherscan(blobHash: string): Promise<{ exists: boolean; blobData?: { txHash: string; blockNumber: number; blobIndex: number; etherscanUrl: string } }> {
		try {
			...
			const result = (data as { result?: { hash?: string; blockNumber?: string; blobVersionedHashes?: readonly string[] } }).result;
			if (result && result.blobVersionedHashes) {
				return {
					exists: true,
					blobData: {
